...
export const CreateShuffleRoomPopup = () => {
  ...
  const getTokenIndex = (tokenTicker) => {
    return tokens.findIndex((token) => token.ticker === tokenTicker);
  };

  const create = async () => {
    let body = { ...roomData };
    const tokenIndex = getTokenIndex(body.tokenTicker);
    try {
      ...
      body.minimumAmount *= 10 ** tokens[tokenIndex].decimals;
      body.maximumAmount *= 10 ** tokens[tokenIndex].decimals;
    } catch (err) {
      console.log(err);
      return;
    }
  ...
  return (
    <Container className="max-w-[460px]">
      <span className="mr-auto font-extrabold text-light">Create Room</span>
      ...
      <div className="mt-3 flex w-full flex-grow items-center">
        ...
        <div className="flex w-1/3 flex-col items-center">
          <label className="mx-auto mr-auto mb-0.5 pr-5 text-center text-sm font-bold">
            <span className="text-center text-mute">Creator Fee</span>
          </label>
          <div className="flex h-11 w-full items-center">
            <Select
              className="mx-auto"
              setState={(points) => {
                setRoomData((prevData) => ({
                  ...prevData,
                  creatorFeeBasisPoints: parseInt(points.replace(/\D/g, "")),
                }));
              }}
              state={
                (roomData.creatorFeeBasisPoints / 100).toFixed(2).toString() +
                "%"
              }
      ...
      <div className="mt-3 flex w-full flex-grow items-center">
        <div className="mr-2 flex w-1/2 flex-col items-center">
          <label className="mr-auto mb-0.5 text-sm font-bold">
            <span className="text-mute">Minimum Amount</span>
          </label>
          <input
            value={roomData.minimumAmount}
            onChange={(e) => {
              setRoomData((prevData) => ({
                ...prevData,
            ...
            className={`h-11 w-full
						rounded-xl bg-[#393869] p-2.5 ${
              roomData.minimumAmount > 0 ? "text-light" : "text-mute"
            }
        ...
        <div className="ml-2 flex w-1/2 flex-col items-center">
          <label className="mr-auto mb-0.5 text-sm font-bold">
            <span className="text-mute">Maximum Amount</span>
          </label>
          <input
            value={roomData.maximumAmount}
            onChange={(e) => {
              setRoomData((prevData) => ({
                ...prevData,
            ...
            className={`h-11 w-full
						rounded-xl bg-[#393869] p-2.5 ${
              roomData.maximumAmount < roomData.minimumAmount &&
              roomData.maximumAmount != 0
                ? "text-red"
                : roomData.maximumAmount > 0
                ? "text-light"
                : "text-mute"
...
export const CreateDuelflipRoomPopup = () => {
  const wallet = useWallet();
  ...
  useEffect(() => {
    (async () => {
      try {
        // const resp = await axiosInstance.get(`${import.meta.env.VITE_API}/prices`);
        // setPrices(resp.data);
        setPrices(roomPrices);
      } catch (err) {
        console.log(err);
      }
    })();
        ...
        <div className="flex w-1/3 flex-col items-center">
          ...
          <div className="flex h-11 w-full items-center">
            <Select
              ...
              state={roomData.price.toString() + " SOL"}

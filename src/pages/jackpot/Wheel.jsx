...
import { adjustSpinValue, formatToken } from "../../util/util";
...
const ProfileImage = ({ user }) => {
  ...
  return (
    <img
      ...
      src={user.profile.image && !imgError ? user.profile.image : DefaultAvatar}
      onError={() => {
        setImgError(true);
      }}
    />
...
const Wheel = ({ result }) => {
  ...
  useEffect(() => {
    ...
    const parts = roomQuery.data.session.users.map((user, i) => {
      ...
      const addZero = i === 0 ? "" : "0 ";
      const percentage =
        lastPercentage + (user.value * 100) / roomQuery.data.session.value;
  ...
  useEffect(() => {
    ...
    let deg = 3600;
    let spinValue = adjustSpinValue(
      roomQuery.data.session.users,
      roomQuery.data.session.value,
      result.spinValue
    );
    deg += 360 - (spinValue * 360) / 10000;
          ...
          >
            {!roomQuery.isSuccess || !roomQuery.data.session.users
              ...
              : (() => {
                  ...
                  return roomQuery.data.session.users.map((user) => {
                    const percentage =
                      lastPercentage +
                ...
                roomQuery.data.session.users.forEach((user) => {
                  ...
                  elements.push(
                    <div
                      ...
                      style={{
                        rotate: `${
                          (360 * (percentage - periodPercentage / 2)) / 100
                        }deg`,
                      }}
        ...
        >
          <div className="m-auto flex flex-col items-center">
            <div className="flex flex-col text-center">
              <span className="text-sm font-bold text-mute">Total Value</span>
            ...
            <div className="-z-10 mb-2">
              <WinChance
                ...
                className="4xs:h-7 3.5xs:h-10 3xs:h-14 2xs:h-20 sm:h-24 md:h-20 md2:h-28 lg:h-24"
              />
            </div>
                    ...
                    !roomQuery.isSuccess
                      ...
                      : (amt) => {
                          setAssets((prevData) => {
                            let clone = [...prevData];
                            if (
                              clone.filter((asset) => asset.type == "Token")
                                .length === 0
                            ) {
                              clone.push({
                                type: "Token",
                                price: amt,
                                mint: roomQuery.data.token.publicKey,
                              });
                              return clone;
                            }
                            clone = clone.map((asset) => {
                              if (asset.type == "Token") {
                                asset.price = amt;
                              }
                              return asset;
                  ...
                  state={
                    roomQuery.isSuccess
                      ? calculateAssetsValue(
                          assets.filter((asset) => asset.type == "Token")
            ...
            <BetButton className="mt-4 hidden 1.5xl:flex" includeNFTs={true} />
          </div>
        </div>
      </div>
    </AspectRatio.Root>

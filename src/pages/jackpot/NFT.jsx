...
import { readableToken, solToken } from "../../util/util";
      ...
      <div className="absolute bottom-0 h-14 w-full ">
        ...
        >
          <span className="overflow-hidden overflow-ellipsis  whitespace-nowrap text-sm font-bold">
            {metadataQuery.isSuccess ? metadataQuery.data.name : "Loading..."}
          </span>
          <span className="flex items-center text-sm font-semibold text-mute">
            <TokenLogo className=" !h-[18px] !w-[18px]" {...solToken} />
            <span className="ml-1">{readableToken(price, solToken)}</span>
          </span>
          <div
          ...
          >
            <span className="text-lg font-bold">
              {!selected ? "+" : <img className="h-4 w-4" src={Check} />}
            </span>
          </div>
        </div>
      </div>

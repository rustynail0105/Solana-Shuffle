...
import DefaultAvatar from "./assets/defaultAvatar-low.png";
...
const Main = () => {
  ...
  const calculatePercentages = () => {
    if (userQuery.isSuccess) {
      console.log("ddfd", userQuery.data);
      ...
      setWinPercentage(
        (userQuery.data.stats.totalLoss / userQuery.data.stats.totalGames) * 100
      );
      setLossPercentage(
        (userQuery.data.stats.totalWin / userQuery.data.stats.totalGames) * 100
      );
    }
  };
      ...
      <div className="h-56 w-full">
        {userQuery.isSuccess ? (
          <img
            ...
            src={userQuery.data.banner ? userQuery.data.banner : DefaultBanner}
            alt="banner"
          />
        ) : (
      ...
      <div className="relative z-10 -mt-12 flex w-full flex-col rounded-[25px] border-t-4 border-[#2F2E5F] bg-[#27264E] p-4 shadow-lg">
          ...
          <div className="absolute bottom-4 left-0 right-0 ml-auto mr-auto lg:-bottom-2">
            <div className="mx-auto flex flex-col items-center">
              <Avatar2
                ...
                level={userQuery.isSuccess && userQuery.data.level}
              />
            </div>
          </div>
        ...
        <div className="mt-6 flex items-center lg:hidden">
          <div className="mx-auto flex flex-col gap-2 text-center">
            <span className="text-xs font-semibold text-mute">Total Games</span>
            <span className="text-2xl font-semibold">
              {userQuery.isSuccess ? userQuery.data.stats.totalGames : 0}
            </span>
          </div>
          <div className="mx-auto h-full w-0.5 bg-[#393869]"></div>
          <div className="mx-auto flex flex-col gap-2 text-center">
            <span className="text-xs font-semibold text-mute">Win</span>
            <span className="text-2xl font-semibold">
